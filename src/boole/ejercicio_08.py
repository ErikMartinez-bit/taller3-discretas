"""
Ejercicio 8 — Simplificación booleana (Quine-McCluskey)
Bloque: Álgebra de Boole / Shannon

Idea matemática:
Un MINTÉRMINO es un término producto donde aparecen TODAS las variables de
la función, cada una complementada o no, correspondiente a exactamente una
fila de la tabla de verdad donde la función vale 1. Por ejemplo, con
variables A,B,C, el mintérmino 5 = binario "101" representa
(A AND NOT B AND C). La "expresión original" es simplemente la suma (OR)
de todos esos mintérminos: por construcción, tiene la misma tabla de
verdad que la función deseada.

El método de Quine-McCluskey simplifica esa suma agrupando mintérminos que
difieren en exactamente un bit: dos términos como "101" y "111" se pueden
combinar en "1-1" (el bit que cambia se vuelve "-", indicando que esa
variable no afecta el resultado en ese grupo — es la ley de absorción /
combinación: X·Y + X·¬Y = X). Esto se repite hasta que ningún término se
pueda combinar más; los términos que sobreviven sin combinarse son los
"implicantes primos". Luego se eligen los mínimos implicantes primos
necesarios para cubrir todos los mintérminos originales (los que cubren un
mintérmino en solitario son "esenciales" y se toman siempre; el resto se
cubre con una heurística voraz, igual que en el coloreo del ejercicio 6).

Dos expresiones booleanas son equivalentes si y solo si tienen la MISMA
tabla de verdad: una tabla de verdad define completamente a una función
booleana (no importa cómo esté escrita la expresión, con más o menos
compuertas), así que comparar tablas de verdad, fila por fila, es la forma
exacta de comprobar que la simplificación no cambió el comportamiento del
circuito, solo lo hizo más barato (menos compuertas).

Nota: no se usó ninguna librería externa (ni sympy); tanto la
simplificación como la verificación de equivalencia están implementadas
